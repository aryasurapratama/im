...
for ($i=0; $i <1000 ; $i++) { 
    ...
    $cekuser = curl('https://api.im2017.com/api/userinfo',null, $token);
    if(strpos($cekuser,'"ok"')){
        ...
        echo  color('green',"[" . date("H:i:s") . "] ")."saldo : $saldo\n";
            ...
            ganjil:
            $addcart = curl("https://api.im2017.com/api/cart/add",'{"productId":"35","cartType":"Custom","cartNum":1,"new":1,"uniqueId":""}',$token);
            if(strpos($addcart,'"ok"')){
                $responen = json_decode($addcart);
                $cartId = $responen->data->cartId;
                echo  color('green',"[" . date("H:i:s") . "] ")."ID : $cartId\n";
                $confirm = curl("https://api.im2017.com/api/order/confirm",'{"cartId":"'.$cartId.'"}',$token);
                $kodetrx = json_decode($confirm)->data->orderKey;
                $createorder = curl("https://api.im2017.com/api/order/create_new/$kodetrx",'{"real_name":"","phone":"","useIntegral":0,"couponId":0,"payType":"yue","pinkId":0,"seckill_id":0,"combinationId":0,"bargainId":0,"from":"weixinh5","mark":"","shipping_type":1}',$token);
                ...
                $trxganjil = curl("https://api.im2017.com/api/guess_odd/saveOrder",'{"order_id":"'.$orderIdfinal.'","guess":1}',$token);
            }else{
            ...
            if(strpos($addcart,'"ok"')){
                ...
                // print_r($cartIds);
                $confirms = curl("https://api.im2017.com/api/order/confirm",'{"cartId":"'.$cartIds.'"}',$token);
                $kodetrxs = json_decode($confirms)->data->orderKey;
                $createorders = curl("https://api.im2017.com/api/order/create_new/$kodetrxs",'{"real_name":"","phone":"","useIntegral":0,"couponId":0,"payType":"yue","pinkId":0,"seckill_id":0,"combinationId":0,"bargainId":0,"from":"weixinh5","mark":"","shipping_type":1}',$token);
                ...
                $trxgenap = curl("https://api.im2017.com/api/guess_odd/saveOrder",'{"order_id":"'.$orderIdfinals.'","guess":0}',$token);
            }else{
                echo  color('red',"[" . date("H:i:s") . "] ")."ID : gagal dibuat\n";
            ...
            $listtrx = curl("https://api.im2017.com/api/order/list?page=1&limit=20&type=1",null,$token);
            ...
            if($timeopentombok == date("H:i:s")){
                ...
                foreach ($response->data as $key){
                    if($key->_game_state === 'Promosi sukses'){
                        $claimreward = curl("https://api.im2017.com/api/order/game_refund",'{"order_id":"'.$key->order_id.'"}',$token);
                        echo  color('green',"[" . date("H:i:s") . "] ")."trx Id : $key->order_id [ status : Promosi sukses ]\n";
                    
                    }
                    if($key->_game_state === 'Promosi gagal'){
                        $claimreward = curl("https://api.im2017.com/api/order/game_integral",'{"order_id":"'.$key->order_id.'"}',$token);
